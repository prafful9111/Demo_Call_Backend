...
const mapDatabaseRowToFrontend = async (row) => {
    ...
    return {
        ...
        call_duration: row.call_duration || '00:00',
        ...
        // New Patient Feedback Analysis Data (Extracted from analysis JSON)
        NPS_Rating: {
            rating_number: analysis.NPS_Rating?.rating_number || null,
            rating_justification: analysis.NPS_Rating?.rating_justification || null
        },

        Issue_Tickets: Array.isArray(analysis.Issue_Tickets)
            ? analysis.Issue_Tickets.map(ticket => ({
        ...
        Feedback_Summary: Array.isArray(analysis.Feedback_Summary)
            ? analysis.Feedback_Summary
            : [],

        Patient_Requests_Asks: Array.isArray(analysis.Patient_Requests_Asks)
            ? analysis.Patient_Requests_Asks.map(request => ({
        ...
        Exceptional_Highlights_Compliments: Array.isArray(analysis.Exceptional_Highlights_Compliments)
            ? analysis.Exceptional_Highlights_Compliments.map(highlight => ({
                evidence: highlight.evidence || '',
                department: highlight.department || '',
                highlight_description: highlight.highlight_description || ''
            }))
            : []
    };
};
    ...
    try {
        ...
        const listViewData = data.map(row => {
            ...
            return {
                id: row.id,
                created_at: row.created_at,
                nps_rating: analysis.NPS_Rating?.rating_number || null,
                issue_count: Array.isArray(analysis.Issue_Tickets) ? analysis.Issue_Tickets.length : 0,
                highlight_count: Array.isArray(analysis.Exceptional_Highlights_Compliments) ? analysis.Exceptional_Highlights_Compliments.length : 0,
